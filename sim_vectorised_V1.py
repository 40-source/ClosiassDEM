...
# numba est commenté car j'ai des problèmes avec
...
def make_combustible(n_couche, delta_ray, pile_ray, pile_height,
    ...
    z_rand = np.random.uniform(0, pile_height,
                    (centre.shape[0], n_particules))
...
def compet(n_steps, dt, delta_ray, n_couche, pile_ray, pile_height,
           n_particules, min_ray, scale, width, height):

    dossier = (str(n_steps) + str(dt) + str(delta_ray) + str(n_couche)
               + str(pile_ray) + str(n_particules) + str(min_ray)
               + str(pile_height)+'_3')

    if not os.path.exists(dossier):
        os.mkdir(dossier)

    combustible = make_combustible(n_couche, delta_ray, pile_ray, pile_height,
                            n_particules)
...
scale = 10.
# nombre d'itérations dans la simulation
n = 300.
# pas de temps
dt = 0.1
# rayon entre deux rangés cylindrique de barres de combustible
dr = 10.
# nombre de rangé concentrique de barres de combustible
rep = 3.
# rayon des barres de combustible
pile_r = 4.
# hauteur des barres de combustible
pile_h = 12
# nombre de particule de combustible par barre.
n_comb = 120
# distance sous laquelle on considère qu'il y a collision
# entre un neutron et une particule de combustible
min_r = 1.

compet(n, dt, dr, rep, pile_r, pile_h, n_comb, min_r, scale, width, height)
