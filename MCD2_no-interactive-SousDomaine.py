...
def histoire(position,direction,e):
    centreA = np.array([100, 0, 0]) #D1
    centreB = np.array([-100, 0, 0]) #D2
    rayon_echantillonne = 30 # spécifique

    if(distance(centreA,position,rayon_echantillonne) or distance(centreB,position,rayon_echantillonne)):
...
def bach(source, n):
    rayon_echantillonne=1600 #générale
