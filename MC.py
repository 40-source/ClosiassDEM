...
def diffusion(position, direction_p, e):
...
def histoire(position,direction,e):
    ...
    if(chi<Ss/S):
        return diffusion(position,direction,e) #diffusion du noyau
    elif(chi>(Ss+Sc)/S):
...
def bach(source, n, r, dossier):
    rayon_echantillonne=r
    centre = np.array([0, 0, 0])
    for i in range(n):
        future = [histoire(neutron[0], neutron[1], neutron[2]) for neutron in source if distance(centre, neutron[0], rayon_echantillonne)]
...
def start(m,n,r,dossier):
    if not os.path.exists(dossier):
        os.mkdir(dossier)
    source=[]
    for _ in range(m):
        source.append([[0,0,0],direction(),2])
    bach(source,n,r,dossier)

parser = argparse.ArgumentParser(description='PHP-Carlo')
parser.add_argument('n', help='ex 200')
parser.add_argument('m', help='ex 0.1')
parser.add_argument('r', help='ex 10')
parser.add_argument('dossier', help='ex toto')
args = parser.parse_args()

start(int(args.m), int(args.n), float(args.r), str(args.dossier))